#include <stdio.h>
#include <limits.h>
#include "./includes/ft_printf.h"  // Inclui o teu ft_printf ou declara aqui
#include <string.h>
...
void    run_bonus_test(const char *test_name, const char *format, ...)
{
    ...
    va_start(args, format);
    ret_ft_printf = vsnprintf(ft_printf_buff, sizeof(ft_printf_buff), format, args);
    va_end(args);
...
int main(void)
{
    ...
    run_bonus_test("Bônus 12: Largura + Precisão em strings", "%10.5s", "abcdef");
